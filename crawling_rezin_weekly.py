...
id = 0 # index번호 주기 위함
platform = '레빈코믹스' # 플렛폼 이름

for j in range(7): # 월~일 별 웹툰 url
    nw_url = f'https://www.lezhin.com/ko/scheduled?day={j}'
    ...
    for i in range(50):
        try:
            ...
            if j == 0:
                j = 7
                title = driver.find_element_by_xpath(f'/html/body/main/section/div/ul[{j}]/li[{i+1}]/a')
            else:
                title = driver.find_element_by_xpath(f'/html/body/main/section/div/ul[{j}]/li[{i+1}]/a')
...
# 열흘쪽 크롤링
nw_url = f'https://www.lezhin.com/ko/scheduled?day=n'
chromedriver_url = './chromedriver'
driver = webdriver.Chrome(chromedriver_url)
driver.get(nw_url)
...
for i in range(7):
        try:
            sleep(random.uniform(1,1.5))
            title = driver.find_element_by_xpath(f'/html/body/main/section/div/ul[8]/li[{i+1}]/a')
            title.click() # 해당 웹툰 클릭하여 단일 웹툰 페이지로 이동
            sleep(random.uniform(1,1.5))
...
webtoon_data.to_csv('./crawling_data/rezin_weekly.csv', encoding='utf-8-sig')
